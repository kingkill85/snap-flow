...
case "$action" in
  install)
    ...
    cp -a "$repo_root/tools/neo_dev_webhook/"{__init__,project_control,project_worker,codex_runtime,runtime_supervisor,operator_commands,verification,forced_command,independent_review,independent_review_canary,deterministic_gates,gate_exec,gate_scan}.py "$stage/snapflow-neo-controller/neo_dev_webhook/"
    cp -a "$repo_root/tools/neo_dev_webhook/"{__init__,project_control,project_worker,codex_runtime,runtime_supervisor,operator_commands,verification,forced_command,independent_review,independent_review_canary,deterministic_gates,gate_exec,gate_scan}.py "$stage/snapflow-neo-controller/"
