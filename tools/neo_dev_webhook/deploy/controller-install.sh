...
install_scope() {
  ...
  id neo-controller >/dev/null 2>&1 || { useradd --system --create-home --shell /bin/sh neo-controller; passwd -l neo-controller; }
  ...
  install -o root -g root -m 0644 "$bundle/neo_dev_webhook/"{__init__,project_control,codex_runtime,verification,forced_command}.py /usr/local/lib/neo_dev_webhook/
...
verify_scope() {
  command -v git >/dev/null; command -v tmux >/dev/null; command -v codex >/dev/null; command -v python3 >/dev/null; command -v sudo >/dev/null
  ...
  test "$(git -C /workspace/snap-flow rev-parse --show-toplevel)" = /workspace/snap-flow
  origin=$(git -C /workspace/snap-flow remote get-url origin); test "$origin" = git@github.com:kingkill85/snap-flow.git || test "$origin" = https://github.com/kingkill85/snap-flow.git
