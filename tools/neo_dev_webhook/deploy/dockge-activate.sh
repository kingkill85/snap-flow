...
verify_scope() {
  local scope=${1:-$stack} compose services receiver_id consumer_id mounts id
...
case "$action" in
  verify) verify_scope ;;
  activate) compose=$stack/compose.yaml; verify_scope; stamp=$(date -u +%Y%m%dT%H%M%SZ); install -d "$backup_root/$stamp"; cp -a "$compose" "$backup_root/$stamp/compose.yaml"; docker compose -f "$compose" ps --format json >"$backup_root/$stamp/containers.json"; (cd "$backup_root/$stamp" && sha256sum compose.yaml containers.json >SHA256SUMS); docker compose -f "$compose" up -d --no-deps --force-recreate receiver consumer; verify_scope; printf '%s\n' "$backup_root/$stamp" ;;
