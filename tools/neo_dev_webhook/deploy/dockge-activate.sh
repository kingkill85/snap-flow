...
verify_definition() {
  local compose=${1:-$stack/compose.yaml} services
...
verify_running_definition() {
  local compose=${1:?compose required} service
  docker compose -f "$compose" config >/dev/null
  while IFS= read -r service; do
    test -n "$(docker compose -f "$compose" ps -q "$service")"
...
case "$action" in
  verify) verify_scope ;;
  activate) compose=$stack/compose.yaml; candidate=$stack/compose.next.yaml; test -f "$compose"; docker compose -f "$compose" config >/dev/null; verify_definition "$candidate"; stamp=$(date -u +%Y%m%dT%H%M%SZ); install -d "$backup_root/$stamp"; cp -a "$compose" "$backup_root/$stamp/compose.yaml"; docker compose -f "$compose" ps --format json >"$backup_root/$stamp/containers.json"; (cd "$backup_root/$stamp" && sha256sum compose.yaml containers.json >SHA256SUMS); cp -a "$candidate" "$compose"; docker compose -f "$compose" up -d --remove-orphans --force-recreate; verify_scope; printf '%s\n' "$backup_root/$stamp" ;;
  rollback) compose=$stack/compose.yaml; backup=${2:?backup required}; [[ $backup =~ ^/mnt/marder/docker/dockge/stacks/snapflow-neo-dev-webhook/.neo-dev-backups/[0-9]{8}T[0-9]{6}Z$ ]]; (cd "$backup" && sha256sum -c SHA256SUMS); cp -a "$backup/compose.yaml" "$compose"; docker compose -f "$compose" up -d --remove-orphans --force-recreate; verify_running_definition "$compose" ;;
  fixture-verify) fixture=${2:?fixture stack}; test -f "$fixture/.dockge-scope-fixture"; verify_scope "$fixture" ;;
  fixture-activate) fixture=${2:?fixture stack}; test -f "$fixture/.dockge-scope-fixture"; verify_scope "$fixture"; docker compose -f "$fixture/compose.yaml" up -d --remove-orphans --force-recreate; verify_scope "$fixture" ;;
  *) echo "usage: $0 <verify|activate|rollback BACKUP|fixture-verify STACK|fixture-activate STACK>" >&2; exit 2 ;;
esac
