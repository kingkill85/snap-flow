...
- Widen the existing Edit Area dialog when applicable parameters exist and present a compact, dense zoning pane with accessible ordered Product Type sections and a reusable application-styled number control composed of explicit decrement and increment actions around a direct-entry value field.
...
- Close the production data/geometry adapter gap demonstrated by an existing project whose persisted positive values appear in Edit Area but produce neither interactive SVG annotations nor PNG paint; both renderers must consume the same real API-loaded Area state, not only synthetic descriptor fixtures.
...
- Frontend: Product Type management subtable/modal controls, Area service/types/state, a reusable compact compound number control built from the application's existing UI primitives, a responsive `AreaEditModal`, production Area data/geometry adapters, a shared deterministic zoning-annotation model/layout, SVG Area annotation rendering, and annotation drawing in the existing PNG export service; no new endpoint, external number-control dependency, export control, or Create Version interaction.
- Tests: Deno repository/route tests, deterministic Vitest component/service/export tests, and Issue #89-tagged Cucumber scenarios executed by Playwright against the real frontend/backend, including the existing Create Version flow, copied-value isolation, compound-control keyboard/bounds/responsive behavior, and same-values DOM plus inspected PNG evidence through a real existing-project path.
