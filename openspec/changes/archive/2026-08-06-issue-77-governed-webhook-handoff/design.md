...
8. Archive first requires strict OpenSpec validation, then runs `tools/openspec_archive_guard.py`; malformed or unsynced delta specs have no confirmation bypass, including the generated archive workflow. For a no-delta change, the guard obtains OpenSpec status JSON itself and accepts only a skipped specs artifact with no existing delta paths.
...
Production invokes the private runner configured by `NEO_DEV_TASK_RUNNER` with safe argv and no shell. Its real contract is a positional title followed by `--body`, bounded configurable `--max-runtime` (default `2h`), valid `--workspace scratch`, and `--idempotency-key`; `--help` is checked for those options. Durable task creation is the handoff boundary. The wrapper requires an idempotency key, reconciles uncertain create outcomes against the durable store, emits one stable `task_id` JSON document, and performs only a bounded best-effort dispatcher wake-up; the gateway's embedded dispatcher provides eventual liveness. The runner requires one stable JSON document with `durable: true` and an explicit `task_id`, then persists that ID. Tests emulate the current help/output shape and never invoke a real task. HTTP concurrency is admitted by a bounded server semaphore before handler-thread allocation or request-body reads, with an absolute per-request deadline and the receiver limit retained as defense in depth.
