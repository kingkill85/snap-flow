...
- Persist the exact Codex session UUID and a controller-owned structured execution state. Correctable findings steer the active process/session; an exited but resumable worker continues through `codex resume <session-id>`; and a fresh Codex session is permitted only for a genuine crash or missing/unusable resumable session, at most once for the governed work identity.
- Separate process exit from semantic completion. Exit code zero records only process termination; success requires an explicit trusted structured completion decision, and blocker/correctable/invalid terminal outcomes never become success through textual output or exit status.
