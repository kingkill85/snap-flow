...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   npm exec -- openspec instructions archive --change "<name>" --json
   ```
...
4. **Assess delta spec sync state**
   ...
   **Prompt options:**
   ...
   `npm exec -- openspec instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   **Strict validation and hard guard (mandatory, no override):** First run
   ...
   The guard obtains and parses OpenSpec status JSON itself when no delta files exist;
   it passes only when the specs artifact is `skipped` with no existing spec paths.
   A missing guard, non-zero result, malformed delta, or unsynced delta
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `npm exec -- openspec archive`).
