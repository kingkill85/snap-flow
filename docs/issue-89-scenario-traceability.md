...
| Long and numerous values | frontend + cucumber | `frontend/src/components/configurator/zoning-annotation.test.ts` proves the exact eight-row/product fixture at 25%, image containment without the product, a fitted effective scale below 25%, and name/prior-annotation separation while preserving canonical anchor/omission parity; real-runtime scenario `Annotation overflow remains bounded and accessible` compares every painted SVG annotation with products, Area names, prior annotations, and image bounds at 25%, fitted-below-25%, 50%, 100%, and 150% |
