...
The persisted resolution contains controller-observed execution and lifecycle state. Controller code is root-owned; state is `neo-controller:neo-controller 0700`. Only the locked `neo-controller` identity has narrow lifecycle sudo rights. Codex remains `dev`, has no lifecycle sudo rule, and cannot read state, replace supervisor code, or invoke lifecycle controls.
...
Process status and worker JSON are never lifecycle authority. The controller alone advances `label → specification_ready → spec_approved → implementation_verified → accepted → archive_ci_verified → merge_authorized → merged_closed`, persisting exact spec, implementation/acceptance, and archive/CI SHAs plus trusted command timestamps. Specification verification rejects every changed path outside the issue's planning tree and narrowly named evidence docs. `/accept` cannot skip approval/review; `/merge` must postdate verified archive/CI state and match that exact head SHA. Closure uses durable synchronization backoff and releases concurrency only after verified `merged_closed` state.
