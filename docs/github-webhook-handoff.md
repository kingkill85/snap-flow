...
`start` first records `starting`, then launches the initial Codex process. Once the controller has verified the generated session UUID against the governed process/worktree, it records `active`. A correctable finding transitions that active state to `correctable`; `resume` then sends a fixed continuation prompt to the existing Codex pane and process without launching another worker. If the process exited with a usable persisted session, `resume` uses exact argv containing `codex resume <session-uuid>`—never `--last`, a picker, or a caller-selected session.
...
Process status and semantic completion are deliberately separate. Exit code zero does not mean the work succeeded. Only a trusted structured terminal outcome of `success` produces `semantic_success`; correctable, blocked, crashed, invalid, prose-only, and missing terminal results remain non-success states. The adapter never parses worker prose to decide completion.
