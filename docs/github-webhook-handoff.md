...
The supervisor is installed mode `0750` below the adapter's private library directory. It is not listed as a card capability, accepts no repository/project/tmux/worktree/branch/worker/path/outcome arguments, and is invoked only by exact argv assembled by `neo-dev-project-control`. It is a lifecycle boundary around the sole Codex worker, not another worker or a second card capability.
