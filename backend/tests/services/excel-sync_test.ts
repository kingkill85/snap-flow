...
import { getNormalUploadRoot } from "../test-runtime-bootstrap.ts";

// Setup test database before all tests
await setupTestDatabase();

function assertNoCommittedMutations(
  result: Awaited<ReturnType<typeof excelSyncService.syncCatalog>>,
): void {
...
Deno.test("ExcelSyncService - import only deactivates missing items from the selected product type", async () => {
  ...
  const sharedCategory = await categoryRepository.create({
    name: "Shared Category",
  });
  const emptyAfterImportCategory = await categoryRepository.create({
  ...
  try {
    ...
    const missingImportedAfter = await itemRepository.findById(
      missingImportedItem.id,
    );
    const protectedAfter = await itemRepository.findById(protectedItem.id);
    const importedExistingAfter = await itemRepository.findById(
      importedExistingItem.id,
    );
    const sharedCategoryAfter = await categoryRepository.findById(
      sharedCategory.id,
    );
    const emptyCategoryAfter = await categoryRepository.findById(
      emptyAfterImportCategory.id,
    );

    console.log(
      "EXCEL_SYNC_OUTCOME " + JSON.stringify({
        databaseTarget:
          getDb().queryEntries<{ file: string }>("PRAGMA database_list")[0]
            .file || ":memory:",
...
Deno.test("ExcelSyncService - embedded images stay inside isolated upload root and cleanup preserves normal uploads", async () => {
  clearDatabase();
  ...
  const workbook = xlsx.utils.book_new();
  xlsx.utils.book_append_sheet(workbook, worksheet, "Catalog");
  const relativePath = "imports/embedded-image-isolation-test.xlsx";
  const fullPath = `${env.UPLOAD_DIR}/${relativePath}`;
  const stagingRoot = `${env.UPLOAD_DIR}/embedded-image-staging`;
  const imageFilename = "issue84-embedded-isolation.png";
  const isolatedImagePath =
    `${env.UPLOAD_DIR}/items/excel-import/${imageFilename}`;
  const normalRoot = getNormalUploadRoot();
  const normalSentinel = `${normalRoot}/issue84-image-sentinel`;
  await Deno.mkdir(`${env.UPLOAD_DIR}/imports`, { recursive: true });
  await Deno.writeFile(
    fullPath,
    new Uint8Array(xlsx.write(workbook, { type: "array", bookType: "xlsx" })),
  );
  await Deno.mkdir(`${stagingRoot}/xl/drawings/_rels`, { recursive: true });
  await Deno.mkdir(`${stagingRoot}/xl/media`, { recursive: true });
  await Deno.writeTextFile(
    `${stagingRoot}/xl/drawings/drawing1.xml`,
    `<xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><xdr:oneCellAnchor><xdr:from><xdr:col>2</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>3</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:pic><xdr:blipFill><a:blip r:embed="rId1"/></xdr:blipFill></xdr:pic></xdr:oneCellAnchor></xdr:wsDr>`,
  );
  await Deno.writeTextFile(
    `${stagingRoot}/xl/drawings/_rels/drawing1.xml.rels`,
    `<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/${imageFilename}"/></Relationships>`,
  );
  ...
  await Deno.writeFile(`${stagingRoot}/xl/media/${imageFilename}`, pngBytes);
  const zipResult = await new Deno.Command("python3", {
    args: [
      "-c",
      "import pathlib,sys,zipfile; archive=zipfile.ZipFile(sys.argv[1], 'a'); root=pathlib.Path(sys.argv[2]); [archive.write(path, path.relative_to(root)) for path in root.rglob('*') if path.is_file()]; archive.close()",
      fullPath,
      stagingRoot,
    ],
  }).output();
  assertEquals(
    zipResult.success,
    true,
    new TextDecoder().decode(zipResult.stderr),
  );
  await Deno.mkdir(normalRoot, { recursive: true });
  await Deno.writeTextFile(normalSentinel, "preserve-me");
  ...
  try {
    ...
    assertEquals(result.success, true);
    assertEquals(result.phases.variants.imagesExtracted, 1);
    assertEquals((await Deno.stat(isolatedImagePath)).isFile, true);
    await assertRejects(
      () => Deno.stat(`${normalRoot}/items/excel-import/${imageFilename}`),
      Deno.errors.NotFound,
    );

    await cleanupTestUploads();
    await assertRejects(
      () => Deno.stat(isolatedImagePath),
      Deno.errors.NotFound,
    );
    assertEquals(await Deno.readTextFile(normalSentinel), "preserve-me");
  } finally {
    await Deno.remove(normalSentinel).catch(() => {});
  }
});
...
Deno.test("ItemRepository - missing-item mutation is constrained to the selected type", async () => {
  ...
  assertEquals(
    Boolean((await itemRepository.findById(selectedMissing.id))?.is_active),
    false,
  );
  assertEquals(
    Boolean((await itemRepository.findById(selectedPresent.id))?.is_active),
    true,
  );
  assertEquals(
    Boolean((await itemRepository.findById(protectedItem.id))?.is_active),
    true,
  );
  assertEquals(
    Boolean(
      (await itemRepository.findById(selectedWithoutModel.id))?.is_active,
    ),
    false,
  );
});
...
Deno.test("ExcelSyncService - invalid type rolls back without broadening scope", async () => {
  ...
  try {
    ...
    assertEquals(
      Boolean((await categoryRepository.findById(category.id))?.is_active),
      true,
    );
  } finally {
    await Deno.remove(fullPath).catch(() => {});
...
Deno.test("ExcelSyncService - empty or unreadable workbook leaves catalog unchanged", async () => {
  ...
  try {
    const emptyResult = await excelSyncService.syncCatalog(
      relativePath,
      selectedType.id,
    );
    const unreadableResult = await excelSyncService.syncCatalog(
      "imports/does-not-exist.xlsx",
      selectedType.id,
    );
    ...
    assertEquals(
      Boolean((await itemRepository.findById(existingItem.id))?.is_active),
      true,
    );
    assertEquals(
      Boolean((await categoryRepository.findById(category.id))?.is_active),
...
Deno.test("ExcelSyncService - caught item persistence failure aborts and rolls back", async () => {
  ...
  getDb().query(
    "CREATE TRIGGER fail_item_update BEFORE UPDATE ON items BEGIN SELECT RAISE(ABORT, 'forced item failure'); END",
  );
  ...
  try {
    ...
    assertNoCommittedMutations(result);
    assertEquals(
      Boolean((await itemRepository.findById(importedItem.id))?.is_active),
      false,
    );
    assertEquals(
      Boolean((await itemRepository.findById(missingItem.id))?.is_active),
      true,
    );
  } finally {
...
Deno.test("ExcelSyncService - caught variant persistence failure aborts and rolls back", async () => {
  ...
  getDb().query(
    "CREATE TRIGGER fail_variant_insert BEFORE INSERT ON item_variants BEGIN SELECT RAISE(ABORT, 'forced variant failure'); END",
  );
...
Deno.test("ExcelSyncService - variant addons can be linked", async () => {
  ...
  const { variantAddonRepository } = await import(
    "../../src/repositories/variant-addon.ts"
  );
