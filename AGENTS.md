...
Before `kanban_complete`, Neo Dev must run the deployed phase reconciler for the exact repository, linked Issue, and internal phase, and require its verified JSON success. Internal mappings are `awaiting_input` → `needs-input`; spec, privileged, and merge approval waits → `needs-approval`; `implementation_in_progress` → `in-progress`; `ready_for_review` → `ready-for-review`; and `blocked` or `non_convergent` → `blocked`. Any mutation or verification failure requires `kanban_block` and the gate/task remains incomplete.
